...
double transformMatrix[4][4]; // matriz de transformação composta

void multiply1(double pointA[], double pointB[]) // multiplica uma matriz 4 x 4 por uma matriz-coluna representativa de um ponto ou vector expresso em coordenadas homogéneas
{
	int i, j;
...
void multiply3(double matrixA[][4]) // multiplica duas matrizes 4 x 4
{
	int i, j, k;
	double matrixB[4][4];
...
void identityMatrix() // cria a matriz correspondente à transformação identidade
{
...
void translate(double x, double y, double z) // cria a matriz correspondente à translação e multiplica a matriz de transformação composta pela matriz recém-criada
{
...
void rotateX(double a) // cria a matriz correspondente à rotação em torno do eixo X e multiplica a matriz de transformação composta pela matriz recém-criada
{
...
void rotateY(double a) // cria a matriz correspondente à rotação em torno do eixo Y e multiplica a matriz de transformação composta pela matriz recém-criada
{
...
void rotateZ(double a) // cria a matriz correspondente à rotação em torno do eixo Z e multiplica a matriz de transformação composta pela matriz recém-criada
{
...
void scale(double x, double y, double z) // cria a matriz correspondente à escala e multiplica a matriz de transformação composta pela matriz recém-criada
{
...
void process(void) // lê de stdin um inteiro qualquer (o valor é irrelevante), seguido de um ponto 3D expresso em coordenadas cartesianas, transforma o ponto e escreve o ponto transformado em stdout
{
	int m, i;
...
void main(void) // constrói a matriz de transformação composta pelas transformações elementares indicadas no corpo da função
{
	identityMatrix(); // inicialização
